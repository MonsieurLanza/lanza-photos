...
americano._listen = function(app, options, callback) {
  var server;
  if (options.tls) {
    server = require('https').createServer(options.tls, app);
    return server.listen(options.port, options.host, function(err) {
      return callback(err, server);
    });
  } else if (options.socket) {
    return server = app.listen(options.socket, function(err) {
      return callback(err, server);
    });
  } else {
...
americano.start = function(options, callback) {
  ...
  return americano._new(options, function(err, app) {
    ...
    return app.beforeStart(function(err) {
      ...
      return americano._listen(app, options, function(err, server) {
        if (err) {
          return typeof callback === "function" ? callback(err) : void 0;
        }
        if (typeof app.afterStart === "function") {
